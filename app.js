...
// ─────────────────────────────────────────────────────────────
//  STORAGE — localStorage so data persists across sessions
//  on the SAME device. For shared multi-device sync, connect
//  a backend (Firebase / Supabase / JSONBin).
// ─────────────────────────────────────────────────────────────
let orders = [];

function loadOrders() {
  try {
    const raw = localStorage.getItem('pradeep_orders');
    orders = raw ? JSON.parse(raw) : [];
  } catch(e) { orders = []; }
}

function saveOrders() {
  try { localStorage.setItem('pradeep_orders', JSON.stringify(orders)); }
...
// ─────────────────────────────────────────────────────────────
function checkLogin() {
  if (sessionStorage.getItem('oms_auth') === PASSKEY) {
...
function genId() {
  ...
  return `ORD-${ds}-${String(Math.floor(Math.random()*900)+100)}`;
}
...
function slaInfo(o) {
  ...
  return { st: pct>=100 ? 'breach' : pct>=75 ? 'warn' : 'ok', pct, rem, el };
}
...
function renderAll() {
  ...
  const nc   = document.getElementById('nc-open');
  if (nc) { nc.textContent = open; nc.classList.toggle('show', open > 0); }
}
...
function renderDash() {
  ...
  tbl.innerHTML = `<table><thead><tr><th>Order ID</th><th>Retailer</th><th>Salesman</th><th>Source</th><th>Status</th><th>Priority</th><th>SLA</th><th>Received</th></tr></thead><tbody>${rows}</tbody></table>`;
}
  ...
  board.innerHTML = STATUSES.map(s => {
    ...
    const cards = col.map(o => {
      ...
      return `<div class="order-card ${cls}" onclick="openDetail('${o.id}')">
        <div class="card-top"><span class="card-id">${o.id}</span>${prioBadge(o.priority)}</div>
        <div class="card-retailer">${o.retailer}</div>
        <div class="card-meta">${srcBadge(o.source)} · <b>${o.salesman}</b></div>
        <div class="card-footer">
          <span class="card-time">${fmtDate(o.receivedAt)}</span>
          <span>${slaChip(o)}</span>
        </div>
      </div>`;
    }).join('');
    return `<div class="lane">
      <div class="lane-header">
        <div class="lane-title"><div class="lane-dot" style="background:${s.dot}"></div>${s.label}</div>
        <span class="lane-count">${col.length}</span>
      </div>
      <div class="lane-body">${cards || '<div class="empty-lane">No orders here</div>'}</div>
    </div>`;
  }).join('');
}
...
function renderOrders() {
  ...
  if (!wrap) return;
  if (!list.length) {
    wrap.innerHTML = `<div class="empty-state"><div class="empty-icon">🔍</div><div class="empty-title">${orders.length ? 'No matching orders' : 'No orders yet'}</div><div class="empty-sub">${orders.length ? 'Try adjusting filters' : 'Click <strong>+ New Order</strong> to get started'}</div></div>`;
    return;
  }
  ...
  wrap.innerHTML = `<table><thead><tr><th>Order ID</th><th>Retailer</th><th>Mobile</th><th>Salesman</th><th>Source</th><th>Status</th><th>Priority</th><th>SLA</th><th>Received</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
function openDetail(id) {
  ...
  const flow = STATUSES.map((s, i) => {
    const done = i < si, cur = i === si, cls = done?'done':cur?'cur':'';
...
function saveOrder() {
  ...
  saveOrders();
  closeModal('newModal');
  toast('Order Created', `${document.getElementById('f-id').value} — ${retailer}`, '#1a7a4a');
  renderAll();
}
...
function lookupPortal() {
  ...
  const pubTl = o.timeline.map(t => `
    ...
    <div><div class="tl-status">${t.label}</div><div class="tl-meta">${fmtDate(t.ts)}</div></div></div>`).join('');
  const pending = STATUSES.slice(si+1).map(s => `
    <div class="tl-item" style="opacity:0.3"><div class="tl-dot"></div>
    <div><div class="tl-status">${s.label}</div><div class="tl-meta">Pending</div></div></div>`).join('');
...
function exportPDF() {
  ...
  const rows = orders.map(o => {
    const s = slaInfo(o);
    return `<tr><td>${o.id}</td><td>${o.retailer}</td><td>${o.mobile||'—'}</td><td>${o.salesman}</td><td>${o.source}</td><td>${STATUSES.find(x=>x.key===o.status)?.label||o.status}</td><td>${o.priority}</td><td>${fmtDate(o.receivedAt)}</td><td>${s.st==='done'?'Done':s.st==='breach'?'Breached':s.st==='warn'?'Warning':'On Track'}</td></tr>`;
  }).join('');
  const summary = STATUSES.map(s => `<div style="display:inline-block;margin:0 12px 8px 0;text-align:center"><div style="font-size:22px;font-weight:700">${orders.filter(o=>o.status===s.key).length}</div><div style="font-size:10px;color:#6b6860;text-transform:uppercase">${s.label}</div></div>`).join('');
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Orders Report</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Helvetica Neue',sans-serif;font-size:12px;color:#1a1916;padding:32px}.hdr{border-bottom:2px solid #d4570e;padding-bottom:16px;margin-bottom:20px;display:flex;justify-content:space-between}.brand{font-size:22px;font-weight:700}.brand span{color:#d4570e}.sum{background:#f7f6f3;border-radius:8px;padding:16px 20px;margin-bottom:20px}table{width:100%;border-collapse:collapse;font-size:11px}th{background:#f7f6f3;text-align:left;padding:8px 10px;font-size:10px;font-weight:600;color:#6b6860;text-transform:uppercase;border-bottom:1px solid #e5e2db}td{padding:8px 10px;border-bottom:1px solid #f0ede8}tr:nth-child(even) td{background:#fafaf8}.ftr{margin-top:24px;padding-top:12px;border-top:1px solid #e5e2db;font-size:10px;color:#a8a49d;display:flex;justify-content:space-between}@media print{body{padding:16px}@page{margin:16mm}}</style></head><body><div class="hdr"><div><div class="brand"><span>Pradeep</span> Sales</div><div style="font-size:11px;color:#6b6860;margin-top:3px">Prayagraj, UP · Authorised Distributor</div></div><div style="text-align:right;font-size:11px;color:#6b6860"><div style="font-size:13px;font-weight:600;color:#1a1916">Orders Report</div><div>Generated: ${date}</div><div>Total: ${orders.length}</div></div></div><div class="sum"><div style="font-size:11px;font-weight:600;color:#a8a49d;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">Pipeline</div>${summary}</div><table><thead><tr><th>Order ID</th><th>Retailer</th><th>Mobile</th><th>Salesman</th><th>Source</th><th>Status</th><th>Priority</th><th>Received</th><th>SLA</th></tr></thead><tbody>${rows}</tbody></table><div class="ftr"><div>Pradeep Sales OMS</div><div>${date}</div></div></body></html>`;
...
// ─────────────────────────────────────────────────────────────
//  WINDOW EXPORTS  (after all function definitions)
// ─────────────────────────────────────────────────────────────
